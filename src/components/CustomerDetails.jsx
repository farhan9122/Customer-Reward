...
const CustomerDetails = ({ transactions, customerId, month, year, onBack }) => {
  ...
  return (
    <div>
      ...
      <p><strong>Total Rewards:</strong> {rewards.total} points</p>
      {rewards.monthly && Object.keys(rewards.monthly).map(monthKey => (
        <p key={monthKey}><strong>{monthKey}:</strong> {rewards.monthly[monthKey]} pts</p>
      ))}
      ...
      {txs.length === 0 ? <p>No transactions</p> : (
        <table>
          <thead>
            <tr>
              <th>ID</th><th>Date</th><th>Amount</th><th>Points</th>
            </tr>
          </thead>
          <tbody>
            {txs.map(tx => (
